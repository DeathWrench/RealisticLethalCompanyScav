...
namespace realisticscavenger
{
    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        {
                ModelReplacementAPI.RegisterModelReplacementOverride(typeof(MRFIXEDSCAV));

            }
            if (enableModelAsDefault.Value)
            {
                ModelReplacementAPI.RegisterModelReplacementDefault(typeof(MRFIXEDSCAV));

            }
            var commaSepList = suitNamesToEnableModel.Value.Split(',');
            foreach (var item in commaSepList)
            {
                ModelReplacementAPI.RegisterSuitModelReplacement(item, typeof(MRFIXEDSCAV));
            }
